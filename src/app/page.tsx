...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden" style={{background:'#0e0e0f',color:'#e7e5e7',fontFamily:"'Manrope',sans-serif"}}>
      ...
      {/* HERO */}
      <section className="relative min-h-[85vh] sm:min-h-screen flex items-center justify-center pt-24 pb-12 sm:pb-20 px-4 sm:px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          <div className="animate-fade-in inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full mb-6 sm:mb-8 text-xs font-bold uppercase tracking-widest" style={{background:'rgba(128,151,255,0.1)',border:'1px solid rgba(128,151,255,0.25)',color:'#8097ff'}}>
            ★ Digitale Kundenbindung — Made by Marketif
          </div>

          <h1 className="animate-fade-in font-headline font-black tracking-tighter mb-4 sm:mb-6 leading-[0.9]" style={{fontSize:'clamp(4rem,9vw,6.5rem)'}}>
            <span className="text-white block">Mehr Stammkunden.</span>
            <span className="text-gradient-hero block">Mehr Umsatz.</span>
          </h1>

          <p className="animate-fade-in-delay max-w-2xl mx-auto mb-8 sm:mb-12 text-base sm:text-lg leading-relaxed px-2" style={{color:'#acaaad',opacity:0.8}}>
          ...
          <div className="animate-fade-in-delay-2 flex flex-col sm:flex-row gap-4 justify-center items-center">
            <a href="#kontakt" className="btn-shimmer inline-flex items-center justify-center gap-2 px-10 py-4 sm:py-5 rounded-full font-black text-base sm:text-lg transition-all hover:scale-105 w-full sm:w-auto" style={{background:'#8097ff',color:'#001760',boxShadow:'0 0 40px rgba(128,151,255,0.35)'}}>Jetzt anfragen →</a>
            <a href="#features" className="inline-flex items-center justify-center gap-2 px-10 py-4 sm:py-5 rounded-full font-bold text-base transition-all hover:scale-105 w-full sm:w-auto" style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.1)',color:'#e7e5e7'}}>Mehr erfahren</a>
          </div>

          <div className="mt-10 sm:mt-20 grid grid-cols-3 gap-3 sm:gap-4 max-w-3xl mx-auto">
            {[{v:'10×',l:'Mehr Stammkunden',c:'#8097ff'},{v:'100%',l:'Ohne App für Kunden',c:'#ddb7ff'},{v:'5 Min',l:'Setup-Zeit',c:'#deffab'}].map(s=>(
              <div key={s.l} className="glass-box rounded-[1.2rem] sm:rounded-[2rem] p-4 sm:p-6 text-center">
                <div className="font-headline font-black text-3xl sm:text-4xl mb-1" style={{color:s.c}}>{s.v}</div>
                <div className="text-[10px] sm:text-xs uppercase tracking-widest" style={{color:'#acaaad'}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="features" style={{background:'#131314',scrollMarginTop:'80px'}} className="py-16 sm:py-28 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12 sm:mb-20">
            <span className="text-sm font-bold uppercase tracking-[0.3em] block mb-3 sm:mb-4" style={{color:'#deffab'}}>Was wir bieten</span>
            <h2 className="font-headline font-bold tracking-tight" style={{fontSize:'clamp(2.4rem,5vw,3.5rem)',lineHeight:'1.05'}}>Alles für Ihre <span className="text-gradient">Kundenbindung</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={f.t} className="glass-box rounded-[1.5rem] sm:rounded-[2rem] p-6 sm:p-8 group text-center sm:text-left">
                ...
                <h3 className="font-headline font-bold text-xl sm:text-xl mb-2 sm:mb-3 text-white">{f.t}</h3>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16 items-center">
            ...
            <div className="text-center lg:text-left">
              <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full text-xs font-black uppercase tracking-[0.2em] mb-6 sm:mb-8" style={{background:'rgba(128,151,255,0.1)',border:'1px solid rgba(128,151,255,0.2)',color:'#8097ff'}}>
                ★ Live Kundenbeispiel
              </div>
              <h2 className="font-headline font-black tracking-tighter mb-4 sm:mb-6" style={{fontSize:'clamp(2.4rem,5vw,3.5rem)',lineHeight:'1.05'}}>
                Restaurant <span className="text-gradient-hero">Aroma</span> — Augsburg
              </h2>
              <p className="text-base sm:text-lg mb-8 leading-relaxed" style={{color:'#acaaad'}}>
      ...
      <section style={{background:'#0e0e0f'}} className="py-16 sm:py-28 px-4 sm:px-6 relative overflow-hidden">
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="text-center mb-12 sm:mb-20">
            <span className="text-sm font-bold uppercase tracking-[0.3em] block mb-3 sm:mb-4" style={{color:'#8097ff'}}>Prozess</span>
            <h2 className="font-headline font-bold tracking-tight" style={{fontSize:'clamp(2.4rem,5vw,3.5rem)',lineHeight:'1.05'}}>So einfach <span className="text-gradient">startet es</span></h2>
          </div>
          <div className="space-y-6">
            {[
              ...
              <div key={i.s} className="flex gap-4 sm:gap-6 items-start p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] transition-all" style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)'}}>
                <div className="shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center font-headline font-black text-base sm:text-lg border-2" style={{color:i.c,borderColor:i.c,background:`${i.c}15`}}>{i.s}</div>
                <div>
                  <h3 className="font-headline font-bold text-xl sm:text-xl mb-1 sm:mb-2" style={{color:i.c}}>{i.t}</h3>
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="glass-card rounded-[1.5rem] sm:rounded-[2.5rem] p-6 sm:p-8 md:p-16" style={{border:'1px solid rgba(255,255,255,0.08)'}}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20">
              <div className="text-center lg:text-left">
                <span className="text-sm font-bold uppercase tracking-[0.3em] block mb-4 sm:mb-6" style={{color:'#ddb7ff'}}>Kontakt</span>
                <h2 className="font-headline font-bold tracking-tight mb-4 sm:mb-6" style={{fontSize:'clamp(2.4rem,4vw,3rem)',lineHeight:'1.05'}}>
                  Lassen Sie uns <span className="text-gradient">starten</span>
                </h2>
                <p className="text-base sm:text-lg mb-8 sm:mb-10 leading-relaxed" style={{color:'#acaaad'}}>
