    ...
    <main className="min-h-screen overflow-x-hidden" style={{background:'#0e0e0f',color:'#e7e5e7',fontFamily:"'Manrope',sans-serif"}}>
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-24 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          <div className="animate-fade-in inline-flex items-center gap-2 px-4 py-2 rounded-full mb-8 text-xs font-bold uppercase tracking-widest" style={{background:'rgba(128,151,255,0.1)',border:'1px solid rgba(128,151,255,0.25)',color:'#8097ff'}}>
            ★ Digitale Kundenbindung — Made by Marketif
          </div>

          <h1 className="animate-fade-in font-headline font-black tracking-tighter mb-6 leading-[0.95]" style={{fontSize:'clamp(3rem,8vw,6.5rem)'}}>
            <span className="text-white block">Mehr Stammkunden.</span>
            <span className="text-gradient-hero block">Mehr Umsatz.</span>
          </h1>

          <p className="animate-fade-in-delay max-w-2xl mx-auto mb-12 text-lg leading-relaxed" style={{color:'#acaaad'}}>
          ...
          <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {[{v:'10×',l:'Mehr Stammkunden',c:'#8097ff'},{v:'100%',l:'Ohne App für Kunden',c:'#ddb7ff'},{v:'5 Min',l:'Setup-Zeit',c:'#deffab'}].map(s=>(
              <div key={s.l} className="glass-box rounded-[2rem] p-6 text-center">
                <div className="font-headline font-black text-4xl mb-1" style={{color:s.c}}>{s.v}</div>
                <div className="text-xs uppercase tracking-widest" style={{color:'#acaaad'}}>{s.l}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="features" style={{background:'#131314',scrollMarginTop:'80px'}} className="py-28 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-20">
            <span className="text-xs font-bold uppercase tracking-[0.3em] block mb-4" style={{color:'#deffab'}}>Was wir bieten</span>
            <h2 className="font-headline font-bold tracking-tight" style={{fontSize:'clamp(2rem,5vw,3.5rem)'}}>Alles für Ihre <span className="text-gradient">Kundenbindung</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              {icon:'🏪',c:'#ddb7ff',t:'Multi-Standort fähig',d:'Perfekt für Restaurants, Cafés und Ketten. Jeder Standort bekommt eigenes Dashboard und eigene Scanner.'},
              ...
              <div key={f.t} className="glass-box rounded-[2rem] p-8 group">
                <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl mb-6 group-hover:scale-110 transition-transform" style={{background:`${f.c}15`,border:`1px solid ${f.c}30`}}>{f.icon}</div>
                <h3 className="font-headline font-bold text-xl mb-3 text-white">{f.t}</h3>
                <p className="text-sm leading-relaxed" style={{color:'#acaaad'}}>{f.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* SHOWCASE — Aroma Card */}
      <section id="showcase" className="py-28 px-6 relative overflow-hidden" style={{background:'#000',scrollMarginTop:'80px'}}>
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            {/* Right: Text */}
            <div>
              <div className="inline-flex items-center gap-3 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] mb-8" style={{background:'rgba(128,151,255,0.1)',border:'1px solid rgba(128,151,255,0.2)',color:'#8097ff'}}>
                ★ Live Kundenbeispiel
              </div>
              <h2 className="font-headline font-black tracking-tighter mb-6" style={{fontSize:'clamp(2rem,5vw,3.5rem)'}}>
              ...
              <a href="#kontakt" className="btn-shimmer inline-flex items-center gap-3 font-black px-10 py-5 rounded-full hover:scale-105 transition-all text-base" style={{background:'#8097ff',color:'#001760',boxShadow:'0 0 40px rgba(128,151,255,0.3)'}}>
                Das will ich auch →
              </a>
            </div>
          </div>
      ...
      {/* HOW IT WORKS */}
      <section style={{background:'#0e0e0f'}} className="py-28 px-6 relative overflow-hidden">
        <div className="max-w-4xl mx-auto relative z-10">
          <div className="text-center mb-20">
            <span className="text-xs font-bold uppercase tracking-[0.3em] block mb-4" style={{color:'#8097ff'}}>Prozess</span>
            <h2 className="font-headline font-bold tracking-tight" style={{fontSize:'clamp(2rem,5vw,3.5rem)'}}>So einfach <span className="text-gradient">startet es</span></h2>
          </div>
          <div className="space-y-6">
            {[
              ...
              <div key={i.s} className="flex gap-6 items-start p-6 rounded-[2rem] transition-all" style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)'}}>
                <div className="shrink-0 w-14 h-14 rounded-full flex items-center justify-center font-headline font-black text-lg border-2" style={{color:i.c,borderColor:i.c,background:`${i.c}15`}}>{i.s}</div>
                <div>
                  <h3 className="font-headline font-bold text-xl mb-2" style={{color:i.c}}>{i.t}</h3>
                  <p className="text-sm leading-relaxed" style={{color:'#acaaad'}}>{i.d}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* CONTACT */}
      <section id="kontakt" className="py-28 px-6 relative overflow-hidden" style={{background:'#131314',scrollMarginTop:'80px'}}>
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          <div className="glass-card rounded-[2.5rem] p-8 md:p-16" style={{border:'1px solid rgba(255,255,255,0.08)'}}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
              <div>
                <span className="text-xs font-bold uppercase tracking-[0.3em] block mb-6" style={{color:'#ddb7ff'}}>Kontakt</span>
                <h2 className="font-headline font-bold tracking-tight mb-6" style={{fontSize:'clamp(2rem,4vw,3rem)'}}>
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{background:'rgba(128,151,255,0.1)'}}>
                      <span style={{color:'#8097ff'}}>✉</span>
                    </div>
                    <span className="font-medium text-white">kontakt@marketif.de</span>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full flex items-center justify-center" style={{background:'rgba(221,183,255,0.1)'}}>
                      <span style={{color:'#ddb7ff'}}>🌐</span>
                    </div>
                    <a href="https://marketif.de" target="_blank" className="font-medium text-white hover:underline">marketif.de</a>
                  </div>
                </div>
              </div>
